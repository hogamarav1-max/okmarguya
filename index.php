...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['login_key'])) {
    ...
    if (!$pm) { $err = $res1['error']['message'] ?? "Tokenization Restricted"; echo json_encode(["status" => "DEAD", "msg" => "$err $bin_info ( Ayan🔥 )"]); exit; }
    ...
    echo json_encode(["status" => $st, "msg" => "$m (Credits: $rem_c) $bin_info ( Ayan🔥 )"]); exit;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>⚡ Ayan PREMIUM CHECKER ⚡</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
...
<div class="wrapper">
    <div class="header-flex">
        <h1><i class="fa-solid fa-bolt-lightning"></i> Ayan PREMIUM</h1>
    ...
    async function start() {
        ...
        while (lines.length > 0 && isRunning) {
            ...
            try {
                ...
                if(data.status === "LIVE") { 
                    setTimeout(() => { location.reload(); }, 99999999999999999); 
                }
